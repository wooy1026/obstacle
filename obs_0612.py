...
class ObstacleMove:
    def __init__(self):
        ...
        self.obs_range = 35 #라이다 인식각도(180도 기준 +-)

        self.default_speed = 0.4  # 기본속도 (주행 및 장애물 회피)
        self.default_angle = 0.0  # 기본조향각 (주행 및 장애물 회피)
        self.backward_speed = -0.2 # 후진 속도

        self.OBSTACLE_PERCEPTION_BOUNDARY = 20 # 장애물 인식 점 개수
    ...
    def laser_callback(self, msg):
        self.msg = msg
        self.is_scan = True
        self.obstacle_ranges = [data for data in msg.ranges if 0 < data < self.range_obs]
    ...
    def LiDAR_scan(self):
        ...
        if self.lidar_flag == False:
            self.degrees = [(self.msg.angle_min + (i * self.msg.angle_increment)) * 180 / pi for i in range(len(self.msg.ranges))]
            self.ranges_length = len(self.msg.ranges)
            self.lidar_flag = True

        if self.ranges_length > 0:
            for i, data in enumerate(self.msg.ranges):
                if i < len(self.degrees) and 0 < data < self.range_obs and 180 - self.obs_range < self.degrees[i] < 180 + self.obs_range:
                    print("장애물 거리",data)
                    obstacle.append(i)
                if len(obstacle) > 0:
    ...
    def move_direction(self, last, first):
        if self.direction == "right":
            print("Dir: Right")
            self.center_list_left = list(range(first))
            if self.center_list_left:
                Lcenter = self.center_list_left[min(len(self.center_list_left) - 1, int(floor(first / 2)))]
                center_angle_left = -self.msg.angle_increment * Lcenter
                self.angle = center_angle_left * (180 / pi)
            else:
            ...
            print("Dir: Left")
            self.center_list_right = list(range(last, self.ranges_length))
            if self.center_list_right:
                Rcenter = self.center_list_right[min(len(self.center_list_right) - 1, int(floor(last + (self.ranges_length - last) / 2)))]
                center_angle_right = self.msg.angle_increment * Rcenter
                self.angle = (center_angle_right / 2.5) * (180 / pi)
            else:
                self.angle = self.default_angle
            self.speed = self.default_speed
    ...
    def compare_space(self, first_dst, last_dst):
        print("장애물 처음:", first_dst)
        print("장애물 마지막:", last_dst)
        # print("장애물 평균거리:", self.dist_data)
    ...
    def main(self):
        ...
        if self.manual == 1:
            ...
            if self.is_scan:
                ...
                if self.stop_flag:
                    print("Auto: Stop_flag")
                    drive.linear.x = 0
                    drive.angular.z = 0
                
                elif self.obstacle_exit_flag == False:
                    print("Auto: forward")
                    drive.linear.x = self.default_speed
                    drive.angular.z = self.default_angle
                else:
                    print("Auto: Obs_Move")
